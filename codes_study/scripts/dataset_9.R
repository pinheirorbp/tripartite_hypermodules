...
# NET files
# NETa
NETa=read.table("data/09_BORNEO_plant_herbivore.csv", sep=",", h=T, row.names = 1)
NETa[is.na(NETa)]=0
NETa=as.matrix(NETa)
...
# NETb
NETb=read.table("data/09_BORNEO_herbivore_defender.csv", sep=",", h=T, row.names = 1)
NETb[is.na(NETb)]=0
NETb=as.matrix(NETb)
...
# Correcting one name 
rownames(NETb)[rownames(NETb)=="Margarodidae__sensu_lato"]="Margarodidae_sensu_lato"
